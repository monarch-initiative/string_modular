"""Command line interface for string_modular."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """string_modular CLI."""
    if version:
        from string_modular import __version__
        typer.echo(f"string_modular version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for string_modular."""
    typer.echo("Downloading data for string_modular...")
...
def transform(
    ...
    """Run the Koza transform for string_modular."""
    typer.echo("Transforming data for string_modular...")
